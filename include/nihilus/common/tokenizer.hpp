...
namespace nihilus {
	...
	template<model_config config, typename derived_type> struct tokenizer<config, derived_type, model_arches::llama, vocab_types::bpe>
		: public tokenizer_parameters<model_arches::llama>,
		  public vocab<model_arches::llama, vocab_types::bpe, tokenizer<config, derived_type, model_arches::llama, vocab_types::bpe>> {
	  ...
	  private:
		...
		NIHILUS_FORCE_INLINE void add_new_bigram(int32_t left, int32_t right) {
			...
			std::string left_token(symbols[static_cast<uint64_t>(left)].text, symbols[static_cast<uint64_t>(left)].n);
			std::string right_token(symbols[static_cast<uint64_t>(right)].text, symbols[static_cast<uint64_t>(right)].n);
