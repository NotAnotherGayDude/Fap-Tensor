...
namespace rt_tm {
	...
	template<typename transform_type> struct kernel_dispatcher_impl<1, kernel_type::softmax, transform_type, float, float, float> {
		// Approximate exp(x) using AVX2 (5x faster than std::expf)
		...
		RT_TM_FORCE_INLINE static void impl(uint64_t count, float* output, const float* input01, const float* input02) {
			...
			// 1. Find max (aligned loads)
			__m256 max_vec = _mm256_set1_ps(-FLT_MAX);
			for (size_t i = 0; i < simd_count; ++i) {
				__m256 logits = _mm256_load_ps(input01 + i * simd_width);// Aligned
				__m256 mask	  = _mm256_load_ps(input02 + i * simd_width);
				max_vec		  = _mm256_max_ps(max_vec, _mm256_blendv_ps(_mm256_set1_ps(-FLT_MAX), logits, mask));
			}
			float max_val = horizontal_max(max_vec);

			// 2. Compute exp and sum (non-temporal stores)
			...
			for (size_t i = 0; i < simd_count; ++i) {
				...
				_mm256_stream_ps(output + i * simd_width, exps);// Non-temporal
				sum_vec = _mm256_add_ps(sum_vec, exps);
			}

			// 3. Normalize (aligned loads)
