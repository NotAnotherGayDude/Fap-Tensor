...
namespace rt_tm {
	...
	template<> struct kernel_dispatcher<device_type::cpu, impl_indices{ .cpu_index = 0 }, op_type::mul, 0> {
		RT_TM_FORCE_INLINE static void impl(cpu_op_core_thread_base* params_new) {
			...
			using source01_type = get_type_from_enum<op_type::mul, 0, 0>::type;
			using source02_type = get_type_from_enum<op_type::mul, 0, 1>::type;
			using dst_type		= get_type_from_enum<op_type::mul, 0, 2>::type;
			for (size_t i = 0; i < params->inputs[0].size(); ++i) {
				const source01_type* ptr01	   = static_cast<const source01_type*>(params->inputs[0][i]);
				const source02_type* ptr02	   = static_cast<const source02_type*>(params->inputs[1][i]);
				dst_type* ptr03				   = static_cast<dst_type*>(params->output[i]);
				ptr03[i]					   = ptr01[i] * ptr02[i];
			}
		}
	};
